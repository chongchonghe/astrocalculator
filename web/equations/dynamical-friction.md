...
expressions:
  - name: "Deceleration (subsonic, v_M << sigma)"
    expression: "dv = 4 * sqrt(2*pi) / 3 * G^2 * M * m_p * n * sigma / sigma^3 * log(Coulomb_log)\n dv in cm/s^2"
  ...
  - name: "Deceleration (supersonic, v_M >> sigma)"
    expression: "dv = 4 * pi * G^2 * M * m_p * n / sigma^2 * log(Coulomb_log)\n dv in cm/s^2"
  ...
  - name: "Decay timescale (BH in galaxy core)"
    expression: "t_fric = 19 / Coulomb_log * (r_i / 5)^2 * sigma / 200 * (1e8 / M)\n t_fric in Gyr"
